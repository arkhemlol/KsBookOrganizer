...
function webpackWrapper(watch, test, bench, callback) {
  var webpackOptions = {
  ...
  if(bench) {
    sources.push(path.join(conf.paths.src, '/app/**/*.bench.ts'));
  }
...
gulp.task('scripts:test-watch', ['scripts'], function (callback) {
  return webpackWrapper(true, true, false, callback);
});

gulp.task('scripts:bench', function () {
  return webpackWrapper(false, false, true);
});

gulp.task('scripts:bench-watch', ['scripts'], function (callback) {
